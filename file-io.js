...
function saveConfigurations() {
    ...
    try {
        ...
        const saveData = {
            ...
            // Include combat simulator consumables data
            combatSimulatorConsumables: window.combatSimulator ? window.combatSimulator.consumables : null,
        ...
        try {
            ...
            if (componentsToLoad) {
                ...
                if (loadedData.componentAttributes) {
                    ...
                    if (typeof initializeConsumables === 'function') {
            ...
            if (window.combatSimulator) {
                ...
                // Load consumables data if present
                if (loadedData.combatSimulatorConsumables) {
                    window.combatSimulator.consumables = loadedData.combatSimulatorConsumables;
                    console.log('Loaded combat simulator consumables from configuration file');

                    // Update UI if combat simulator is open
                    if (document.getElementById('combat-simulator-modal')) {
                        // Update standard consumables inputs
                        ['left', 'right'].forEach(fleet => {
                            const consumables = window.combatSimulator.consumables[fleet].standard;
                            Object.keys(consumables).forEach(type => {
                                const input = document.getElementById(`${fleet}-${type}`);
                                if (input) {
                                    input.value = consumables[type];
                                }
                            });

                            // Update stimulants lists
                            if (typeof updateStimulantsList === 'function') {
                                updateStimulantsList(fleet);
                            }
                        });
                    }
                }
