...
// Multiplier stacking mode: 'linear' or 'compound'
let multiplierStackingMode = 'linear'; // Default to current behavior
window.multiplierStackingMode = 'linear'; // Make it globally accessible

// Function to refresh all ship stats
window.refreshAllShipStats = function () {
    console.log('[Refresh Stats] Refreshing all ship stats with', window.multiplierStackingMode, 'stacking mode');
...
function extractComponentCategories() {
    console.log('🔍 extractComponentCategories called, components:', !!components, 'rewardTree:', !!(components && components.rewardTree));
    if (!components || !components.rewardTree) {
        // Even if no components, ensure CONSUMABLES exists
...
function initComponentAttributes() {
    ...
    // Initialize Consumables category with predefined structure
    initializeConsumables();
}

// Initialize CONSUMABLES category with all sub-groups and attributes
function initializeConsumables() {
    console.log('🚀 Initializing Consumables category...');

    // Ensure CONSUMABLES category exists
    if (!componentAttributes['Consumables']) {
        componentAttributes['Consumables'] = {};
    }

    // Define the complete Consumables structure
    const consumablesStructure = {

        // CONSUMPTION RATE STIMULANTS ONLY
        // All stimulants now ONLY affect resource consumption rates
        // All other buffs (damage, counters, etc.) reserved for future "Ship Charges"

        'Ammo Conservation Serum': {
            ammo_consumption_rate: { baseValue: 1.0, type: 'multiplicative', multiplicativeValue: 0.80 }
        },
        'Food Efficiency Optimizer': {
            food_consumption_rate: { baseValue: 1.0, type: 'multiplicative', multiplicativeValue: 0.75 }
        },
        'Fuel Conservation Injection': {
            fuel_consumption_rate: { baseValue: 1.0, type: 'multiplicative', multiplicativeValue: 0.80 }
        },
        'Counter Resource Manager': {
            counter_consumption_rate: { baseValue: 1.0, type: 'multiplicative', multiplicativeValue: 0.85 }
        },
        'Repair Efficiency Boost': {
            repair_cost: { baseValue: 1.0, type: 'multiplicative', multiplicativeValue: 0.85 }
        },
        'Multi-Resource Optimizer': {
            ammo_consumption_rate: { baseValue: 1.0, type: 'multiplicative', multiplicativeValue: 0.90 },
            food_consumption_rate: { baseValue: 1.0, type: 'multiplicative', multiplicativeValue: 0.90 },
            fuel_consumption_rate: { baseValue: 1.0, type: 'multiplicative', multiplicativeValue: 0.90 }
        },
        'Combat Resource Manager': {
            ammo_consumption_rate: { baseValue: 1.0, type: 'multiplicative', multiplicativeValue: 0.85 },
            counter_consumption_rate: { baseValue: 1.0, type: 'multiplicative', multiplicativeValue: 0.85 }
        },
        'Engineering Efficiency Serum': {
            repair_cost: { baseValue: 1.0, type: 'multiplicative', multiplicativeValue: 0.80 },
            fuel_consumption_rate: { baseValue: 1.0, type: 'multiplicative', multiplicativeValue: 0.90 }
        }
    };

    // Populate componentAttributes with the Consumables structure
    for (const [groupName, groupData] of Object.entries(consumablesStructure)) {
        componentAttributes['Consumables'][groupName] = groupData;
    }

    // Add Consumables groups to componentCategories
    componentCategories['Consumables'] = Object.keys(consumablesStructure);
    console.log('✅ Consumables category initialized with groups:', componentCategories['Consumables']);

    // Recreate category buttons to include Consumables
    if (typeof createCategoryButtons === 'function') {
        console.log('🔄 Recreating category buttons...');
        createCategoryButtons();
    } else {
        console.warn('⚠️ createCategoryButtons function not available');
    }
}

// Create category buttons in the header
function createCategoryButtons() {
    console.log('🔧 createCategoryButtons called, componentCategories:', Object.keys(componentCategories));
    const container = document.getElementById('category-buttons');
    if (!container) {
        console.error('❌ category-buttons container not found!');
        return;
    }
    console.log('✅ Found category-buttons container');
    ...
    Object.keys(componentCategories).forEach(category => {
        ...
        console.log(`✅ Added ${category} button to container`);
    });

    console.log('🎯 Finished creating all category buttons');
}
        ...
        if (typeof modifiedStats[statName] === 'number' && statEffects[statName].length > 0) {
            ...
            Object.values(groupedEffects).forEach(group => {
                ...
                if (group.type === 'multiplicative' || group.type === 'both') {
                    if (window.multiplierStackingMode === 'compound') {
                        // Compound multiplication: multiply all instances together
                        for (let i = 0; i < group.count; i++) {
                            totalMultiplier *= group.multiplier;
                        }

                        if (group.multiplier !== 1.0) {
                            const compoundEffect = Math.pow(group.multiplier, group.count);
                            console.log(`${statName}: ${group.count}x ${group.sample.componentName} @ ${group.multiplier} = ${compoundEffect.toFixed(3)}x (compound multiplicative)`);
                        }
                    } else {
                        ...
                        if (multiplicativeEffect !== 0) {
                            console.log(`${statName}: ${group.count}x ${group.sample.componentName} @ ${group.multiplier} = +${(multiplicativeEffect * 100).toFixed(1)}% (linear multiplicative)`);
                        }
                    }
                }
            });
...
function initApp() {
    ...
    // Initialize CONSUMABLES category first
    initializeConsumables();
    ...
    if (typeof initCombatSimulator === 'function') {
        initCombatSimulator().catch(err => {
            console.error('Failed to initialize combat simulator:', err);
        });
    } else {
    ...
    try {
        ...
        if (componentsToLoad) {
            ...
            if (loadedData.componentAttributes) {
                ...
                // Always re-initialize CONSUMABLES after loading to ensure it exists
                initializeConsumables();
            } else {
                if (typeof initComponentAttributes === 'function') {
                    initComponentAttributes();
                }
                // Initialize CONSUMABLES if creating new attributes
                initializeConsumables();
            }
...
// Export initializeConsumables to global scope
window.initializeConsumables = initializeConsumables;
